...
public class Shrapnel extends AbstractHurtingProjectile {
	...
	public static final DamageSource SHRAPNEL = DamageSource.MAGIC;
	...
	public static <T extends Shrapnel> List<T> spawnShrapnelBurst(Level level, EntityType<T> type, Vec3 position, Vec3 initialVelocity, int count, double spread, float damage) {
		...
		for (int i = 0; i < count; ++i) {
			...
			Vec3 vel = forward.scale(velScale).add(right.scale((random.nextDouble() - random.nextDouble()) * velScale * spread)).add(up.scale((random.nextDouble() - random.nextDouble()) * velScale * spread));
	...
	protected boolean canDestroyBlock(BlockState state) {
		return state.is(CBCTags.BlockCBC.SHRAPNEL_SHATTERABLE) || state.is(CBCTags.BlockCBC.SHRAPNEL_VULNERABLE) && this.random.nextFloat() < CBCConfigs.SERVER.munitions.shrapnelVulnerableBreakChance.getF();
	}
