...
public interface BlockRecipeSerializer<T extends BlockRecipe> extends RegistryEntry<BlockRecipeSerializer<?>> {
	...
	class Entry<T extends BlockRecipe> extends RegistryEntry<BlockRecipeSerializer<T>> {
		...
		public T fromJson(ResourceLocation id, JsonObject obj) {
			return this.get().fromJson(id, obj);
		}

		public T fromNetwork(ResourceLocation id, FriendlyByteBuf buf) {
			return this.get().fromNetwork(id, buf);
		}

		public void toNetwork(FriendlyByteBuf buf, T recipe) {
			this.get().toNetwork(buf, recipe);
		}
	}
	...
	class Builder<T extends BlockRecipe, P> extends AbstractBuilder<BlockRecipeSerializer<?>, BlockRecipeSerializer<T>, P, Builder<T, P>> {
		...
		@Override
		protected @NonnullType BlockRecipeSerializer<T> createEntry() {
			return this.factory.get();
		}

		@Override
		public Entry<T> register() {
			return (Entry<T>) super.register();
		}

		@Override
		protected Entry<T> createEntryWrapper(RegistryObject<BlockRecipeSerializer<T>> delegate) {
