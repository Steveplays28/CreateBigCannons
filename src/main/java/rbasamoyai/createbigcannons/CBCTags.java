...
public class CBCTags {
	...
	public static class BlockCBC {
		public static final TagKey<Block> THICK_TUBING = createAndGenerateBlockTag(CreateBigCannons.resource("thick_tubing")), REDUCES_SPREAD = createAndGenerateBlockTag(CreateBigCannons.resource("reduces_spread")), SHRAPNEL_SHATTERABLE = createAndGenerateBlockTag(CreateBigCannons.resource("shrapnel_shatterable")), SHRAPNEL_VULNERABLE = createAndGenerateBlockTag(CreateBigCannons.resource("shrapnel_vulnerable")), GRAPESHOT_SHATTERABLE = createAndGenerateBlockTag(CreateBigCannons.resource("grapeshot_shatterable")), GRAPESHOT_VULNERABLE = createAndGenerateBlockTag(CreateBigCannons.resource("grapeshot_vulnerable")), WEAK_CANNON_END = createAndGenerateBlockTag(CreateBigCannons.resource("weak_cannon_end")), DRILL_CAN_PASS_THROUGH = createAndGenerateBlockTag(CreateBigCannons.resource("drill_can_pass_through")), DEFLECTS_SHOTS = createAndGenerateBlockTag(CreateBigCannons.resource("deflects_shots")), DOESNT_DEFLECT_SHOTS = createAndGenerateBlockTag(CreateBigCannons.resource("doesnt_deflect_shots"));

		public static TagKey<Block> createAndGenerateBlockTag(ResourceLocation loc) {
			TagKey<Block> tag = TagKey.create(ResourceKey.createRegistryKey(loc), loc);
	...
	public static class ItemCBC {
		public static final TagKey<Item> IMPACT_FUZE_HEAD = createAndGenerateItemTag(CreateBigCannons.resource("impact_fuze_head")), NUGGET_CAST_IRON = forgeTag("nuggets/cast_iron"), INGOT_CAST_IRON = forgeTag("ingots/cast_iron"), BLOCK_CAST_IRON = forgeTag("storage_blocks/cast_iron"), NUGGET_BRONZE = forgeTag("nuggets/bronze"), INGOT_BRONZE = forgeTag("ingots/bronze"), BLOCK_BRONZE = forgeTag("storage_blocks/bronze"), NUGGET_STEEL = forgeTag("nuggets/steel"), INGOT_STEEL = forgeTag("ingots/steel"), BLOCK_STEEL = forgeTag("storage_blocks/steel");
		...
		// TODO: Convert to Fabric tag?
		public static TagKey<Item> forgeTag(String path) {
			TagKey<Item> tag = TagKey.create(Registry.ITEM_REGISTRY, new ResourceLocation("forge", path));
			REGISTRATE.addDataGenerator(ProviderType.ITEM_TAGS, prov -> prov.tag(tag));
			return tag;
		}
